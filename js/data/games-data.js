// js/data/games-data.js
const gamesData = {
    "current": [
        {
            "id": "prisn",
            "title": "Prisn",
            "description": "A dark, cel-shaded puzzle-platformer where you play as a wrongfully imprisoned scientist using light and color to escape. Solve environmental puzzles, learn real color theory, and uncover a prison-wide conspiracy hidden in the spectrum.",
            "status": "in-development",
            "releaseDate": "TBD",
            "genre": "Puzzle-Platformer",
            "platforms": ["PC"],
            "tags": ["pc", "puzzle", "story"],
            "features": [
                "Manipulate light to solve security puzzles",
                "Blend pigments to forge tools and unlock paths",
                "Narrative choices that reshape the prison's factions"
            ],
            "images": ["prisn_keyart.svg", "prisn_puzzle.svg"],
            "trailerUrl": "",
            "downloadLinks": {}
        },
        {
            "id": "watertoafish",
            "title": "Water To A Fish",
            "description": "A tongue-in-cheek underwater retail sim where you sell bottled water to picky fish. Restock shelves, manage pricing, deal with thieves, and keep your 5-star rating as your shop scales from humble tank to ocean empire.",
            "status": "in-development",
            "releaseDate": "TBD",
            "genre": "Retail Simulator",
            "platforms": ["PC"],
            "tags": ["pc", "simulation", "management"],
            "features": [
                "Day/night cycle with shifting customer demand",
                "Shelf-stocking puzzles and quirky VIP orders",
                "Upgradeable storefronts across the reef"
            ],
            "images": ["watertoafish_keyart.svg", "watertoafish_shop.svg"],
            "trailerUrl": "",
            "downloadLinks": {}
        },
        {
            "id": "beneaththewaves",
            "title": "Beneath The Waves",
            "description": "A cozy underwater social sim where you befriend sea creatures and help restore the ocean. Catch drifting plastic, assist in conservation, and build homes for aquatic friends in an open-ended, peaceful world.",
            "status": "in-development",
            "releaseDate": "TBD",
            "genre": "Social Simulation",
            "platforms": ["PC"],
            "tags": ["pc", "cozy", "simulation"],
            "features": [
                "Dynamic ocean biomes that react to your care",
                "Friendship quests with expressive marine life",
                "Base-building with sustainable crafting"
            ],
            "images": ["beneaththewaves_keyart.svg", "beneaththewaves_sanctuary.svg"],
            "trailerUrl": "",
            "downloadLinks": {}
        },
        {
            "id": "thelastladle",
            "title": "The Last Ladle",
            "description": "A full text adventure game where you operate the last food truck in a post-apocalyptic world. Scavenge for ingredients, and feed the last of humanity.",
            "status": "in-development",
            "releaseDate": "TBD",
            "genre": "Text Adventure",
            "platforms": ["Web", "PC"],
            "tags": ["web", "cozy", "text-adventure"],
            "features": [
                "Relaxed, session-friendly gameplay",
                "Progression-focused systems",
                "Stylized visuals"
            ],
            "images": ["thelastladle_keyart.svg", "thelastladle_logo.svg"],
            "trailerUrl": "",
            "downloadLinks": {}
        },
        {
            "id": "thelastcircuit",
            "title": "The Last Circuit",
            "description": "A terminal-first, Vite-powered prototype where you stabilize a collapsing network by issuing SCAN, REPAIR, ROUTE, and OPTIMIZE commands through an in-world console.",
            "status": "prototype",
            "releaseDate": "TBD",
            "genre": "Terminal Strategy Prototype",
            "platforms": ["Web"],
            "tags": ["web", "prototype", "terminal"],
            "features": [
                "In-browser, command-driven interface",
                "Dynamic network metrics and entropy management",
                "Lore-leaning field notes unlocked through play"
            ],
            "images": ["thelastcircuit_keyart.svg", "thelastcircuit_logo.svg"],
            "trailerUrl": "",
            "downloadLinks": {}
        },
        {
            "id": "puzzlesuite",
            "title": "Puzzle Suite",
            "description": "A handcrafted browser suite of eighteen classic puzzle games led by Chromogram, a Slowbern original. Jump between nonograms, sudoku, minesweeper, and more with an offline-ready, installable experience.",
            "status": "prototype",
            "releaseDate": "TBD",
            "genre": "Puzzle Collection",
            "platforms": ["Web"],
            "tags": ["web", "puzzle", "collection"],
            "features": [
                "18 classic puzzle modes plus the original Chromogram",
                "Seeded puzzles, hints, and accessibility toggles",
                "Installable PWA built for offline play"
            ],
            "images": ["puzzlesuite_icon.svg"],
            "trailerUrl": "",
            "downloadLinks": {}
        }
    ],
    "expansions": []
};
