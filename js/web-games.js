...
class WebGameManager {
    ...
    renderGamesList() {
        const container = document.getElementById('web-games-container');
        if (!container) {
            return;
        }

        const webGames = (this.games.current || []).filter(game =>
            Array.isArray(game.platforms) &&
            game.platforms.some(platform => String(platform).toLowerCase() === 'web')
        );

        const gamesHTML = webGames.map(game => this.createGameCard(game)).join('');
        container.innerHTML = gamesHTML || '<p class="card">No web games are available yet.</p>';
    }
